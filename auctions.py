...
def fetch_auction_data(url, geo, category):
    s = requests.Session()
    page_number = 1
    all_auctions = []
    while True:
        ...
        with open(
            f"Data_files/{geo}/auctions/data_html/{category}_page_{page_number}.html",
            "w",
            encoding="utf-8",
        ) as file:
        ...
        for item in all_auctions_on_page:
            cleaned_auction_dict = prepear_data(item)
            all_auctions.append(cleaned_auction_dict)
        print(all_auctions)
        page_number += 1
        with open(
            f"Data_files/{geo}/auctions/data_json/{category}.json",
            "w",
            encoding="utf-8",
        ) as file:
            json.dump(
